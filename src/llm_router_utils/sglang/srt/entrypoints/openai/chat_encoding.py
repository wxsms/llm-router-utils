...
def encode_simple_chat(
    *,
    ...
    if spec == "inkling":
        ...
        from llm_router_utils.sglang.srt.parser.inkling_tokenizer import InklingTokenizer
    ...
    if spec in ("dsv4", "dsv32"):
        ...
        if spec == "dsv4":
            from llm_router_utils.sglang.srt.entrypoints.openai import encoding_dsv4

            real_input = encoding_dsv4.encode_messages(
                messages, thinking_mode=thinking_mode
            )
        else:
            from llm_router_utils.sglang.srt.entrypoints.openai import encoding_dsv32
