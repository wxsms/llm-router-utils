...
config classes (AfmoeConfig, BailingHybridConfig, ... DeepseekVL2Config,
InternVLChatConfig, etc.) and the ``import torch`` top-level import have
been removed because llm-router-utils does not ship the inference-engine
config package and must remain torch-free at import time.

The ``_CONFIG_REGISTRY`` dict is kept (empty) so that config.py's
``if config.model_type in _CONFIG_REGISTRY`` checks compile and behave as
no-ops for pure-text model types.  The transformers-internal alias blocks
(DeepseekV3, Mellum, Gemma4) are preserved verbatim -- they self-skip via
try/except ImportError when the transformers version lacks the symbols.
"""
...
from llm_router_utils.sglang.srt.utils import get_bool_env_var, logger, lru_cache_frozenset
from llm_router_utils.sglang.srt.utils.runai_utils import ObjectStorageModel, is_runai_obj_uri
...
# ---------------------------------------------------------------------------
#
# Upstream populates this with ~40 custom config classes from
# ``sglang.srt.configs``.  llm-router-utils does not ship that package, so
# the registry starts empty.  The transformers-internal alias blocks below
# (DeepseekV3, Mellum, Gemma4) still register themselves when the installed
# transformers version exposes the relevant base classes.
...
def resolve_hf_gguf_reference(
    ...
    """
    from llm_router_utils.sglang.srt.utils import is_remote_url
