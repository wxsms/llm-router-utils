...
multimodal-only ``sglang.srt.configs.deepseek_ocr``) is replaced with a
``None`` sentinel so the OCR-specific branches compile but raise
``NameError`` if actually entered (they are never hit on the pure-text path).
The ``create_remote_connector`` import is satisfied by the local stub.
"""
...
from llm_router_utils.sglang.srt.configs.model_config_parser_registry import (
...
from llm_router_utils.sglang.srt.utils import is_remote_url, lru_cache_frozenset
...
# DeepseekVLV2Config lives in the multimodal-only
# ``sglang.srt.configs.deepseek_ocr`` module which llm-router-utils does not
# ship.  The OCR2 code path that uses it is never hit for pure-text models,
# so a ``None`` sentinel is sufficient (it will raise at call time if ever
# reached, which is the desired fail-loud behaviour for an unsupported path).
...
def _try_load_raw_mamba_config(model, revision: Optional[str], **kwargs):
    ...
    Detect that shape and build a Mamba-style config (model_type `mamba`, arch
    ...
    from llm_router_utils.sglang.srt.utils.hf_transformers.common import (
        _MambaConfigAlias as MambaConfig,
    )
